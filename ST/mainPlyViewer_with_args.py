...
def main():
    ...
    # 使い方:
    #   python mainPlyViewer.py                 -> 従来どおり: ファイル選択ウィンドウ
    #   python mainPlyViewer.py <path/to.ply>   -> 指定PLYを直接表示
    ply_arg = None
    if len(sys.argv) >= 2:
        cand = sys.argv[1]
        if isinstance(cand, str) and os.path.isfile(cand) and cand.lower().endswith(".ply"):
            ply_arg = cand

    if ply_arg:
        w = PointCloudViewer(ply_arg)
        w.show()
    else:
        w = PlySelectWindow()
        w.show()
