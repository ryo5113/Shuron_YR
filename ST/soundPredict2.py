...
def split_cleaned_wav_to_folder(cleaned_wav: Path, out_dir: Path, start_index: int, prefix: str, mirror_dir: Path | None = None,) -> list[Path]:
    ...
    ranges = vc.detect_nonsilent(
        audio,
        min_silence_len=vc.MIN_SILENCE_LEN_MS,
        silence_thresh=vc.SILENCE_THRESH_DBFS,
    )
    try:
        if ranges:
            ...
            print(
                ...
                f"count={len(ranges)}, thresh={vc.SILENCE_THRESH_DBFS}, min_silence_len={vc.MIN_SILENCE_LEN_MS}"
            )
        else:
            print(
                f"[detect_nonsilent] no ranges. thresh={vc.SILENCE_THRESH_DBFS}, "
                f"min_silence_len={vc.MIN_SILENCE_LEN_MS}"
            )
    ...
    refined = vc.postprocess_ranges(audio, ranges)

    saved_paths: list[Path] = []
    saved = 0
    for start_ms, end_ms in refined:
        s = max(0, start_ms - vc.KEEP_SILENCE_MS)
        e = min(len(audio), end_ms + vc.KEEP_SILENCE_MS)
        chunk = audio[s:e]

        if vc.is_valid_chunk(chunk):
            saved += 1
...
def extract_voiced_chunks_by_voicecutting(in_wav: Path, out_dir: Path, base_name: str) -> list[Path]:
    """
    voiceCutting.py のロジックで有音区間を抽出し、out_dir に chunk を保存して返す。
    """
    ...
    for (start_ms, end_ms) in refined:
        ...
        if vc.is_valid_chunk(chunk):
            voiced_idx += 1
            out_path = out_dir / f"{base_name}_v{voiced_idx:02d}.wav"
            chunk.export(
                str(out_path),
                format="wav",
                parameters=["-ac", "1", "-ar", "48000", "-acodec", "pcm_s16le"],
...
def split_wav_by_time_to_folder(in_wav: Path, out_dir: Path, segment_ms: int = 1000) -> list[Path]:
    """
    取得した raw.wav から、1〜11秒の1秒おき（= 1-2,2-3,...,10-11 の計10区間）だけを切り出す。
    11秒以降が録音されていても無視する。
    """
    audio = AudioSegment.from_file(str(in_wav))
    out_dir.mkdir(parents=True, exist_ok=True)
    ...
    for i in range(1, 11):  # i=1..10
        ...
        # 録音が11秒未満なら、取れる分だけで終了
        if len(audio) < e:
            break

        seg = audio[s:e]
        p = out_dir / f"seg_{i:04d}.wav"  # seg_0001=1-2s ... seg_0010=10-11s
        seg.export(
            str(p),
            format="wav",
            parameters=["-ac", "1", "-ar", "48000", "-acodec", "pcm_s16le"],
...
def main(page: ft.Page, root_home=None):
    ...
    def on_stop_train_record_after(word: str):
        if not state.is_recording or state.stream is None or state.frames is None:
            set_status("録音中ではありません。")
            return
        if state.current_label != word:
        ...
        def worker():
            try:
                ...
                # 1) raw を 1秒ごとに分割（0-1s,1-2s,...）
                seg_dir = state.raw_dir / f"segments_{state.current_label}_{stamp}"
                seg_paths = split_wav_by_time_to_folder(raw_wav, seg_dir, segment_ms=1000)

                saved_paths: list[Path] = []
                idx = start_index

                # 2) 各1秒wavを denoise → 3) voiceCuttingで有音chunk抽出 → 4) dataset保存
                for seg_i, seg_path in enumerate(seg_paths, start=1):
                    # denoiseした1秒wav（中間ファイル）
                    deno_seg_path = state.clean_dir / f"deno_seg_{state.current_label}_{stamp}_{seg_i:04d}.wav"
                    denoise_wav_to_path(seg_path, deno_seg_path)

                    # voiceCuttingで有音chunkを抽出（denoise済みchunkが出る）
                    tmp_voiced_dir = state.clean_dir / f"voiced_{state.current_label}_{stamp}_{seg_i:04d}"
                    voiced_chunks = extract_voiced_chunks_by_voicecutting(
                        deno_seg_path,
                        tmp_voiced_dir,
                        base_name=f"tmp_{seg_i:04d}"
                    )

                    # voiced_chunks を dataset に保存（命名：{prefix}_{連番}.wav）
                    for vc_path in voiced_chunks:
                        filename = f"{prefix}_{idx}.wav"
                        out_path = out_label_dir / filename
                        out_all_path = out_all_label_dir / filename

                        # copy ではなく dataset に "export" で統一（誤フォルダ参照を避ける）
                        chunk_audio = AudioSegment.from_file(str(vc_path))
                        chunk_audio.export(
                            str(out_path),
                            format="wav",
                            parameters=["-ac", "1", "-ar", "48000", "-acodec", "pcm_s16le"],
                ...
                state.last_train_chunks = list(saved_paths)
                state.last_train_label = state.current_label

                ensure_label_state()
                label_counts[state.current_label] = label_counts.get(state.current_label, 0) + len(saved_paths)
                label_count_view[state.current_label].value = f"取得回数: {label_counts[state.current_label]}"

                set_status(f"録音：{len(saved_paths)}個の保存を行いました。", kind="success")
                update_train_paths()
                page.update()
            ...
            controls=[
                ft.Row(
                    [
                        ...
                        reg_button(ft.ElevatedButton(
                            text="録音停止",
                            on_click=lambda e, w=word: on_stop_train_record_after(w)
                        )),
                        reg_button(ft.ElevatedButton(
                            text="削除",
        ...
        def worker():
            try:
                ...
                with tempfile.TemporaryDirectory() as td:
                    ...
                    vc.SILENCE_THRESH_DBFS = -40  # infer用にしきい値を下げる
