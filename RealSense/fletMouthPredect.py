...
def main(page: ft.Page):
    ...
    preview = ft.Image(src_base64=DUMMY_PNG_B64, width=800, height=450)

    status = ft.Text(value="READY", selectable=True)
    paths_view = ft.Text(value="", selectable=True)
    pred_view = ft.Text(value="PRED: --", selectable=True)
    ...
    infer_parent = ft.TextField(label="推論：親フォルダ名（例：被験者フォルダ）", width=520)
    ...
    def build_train_page():
        return ft.Column(
            [
                ...
                subject_name,
                ft.Row([
                    ft.ElevatedButton(text="親フォルダ作成", on_click=on_create_folder),
                    ft.ElevatedButton(text="収録停止", on_click=on_stop_capture),
                    ft.ElevatedButton(text="学習開始（モデル保存）", on_click=on_train_start),
                ]),
                ft.Row([
                    ft.ElevatedButton(text=f"{lbl} 収録開始", on_click=lambda e, l=lbl: on_start_capture_for_label(l))
                    for lbl in LABELS
                ]),
                ft.Divider(),
                ft.Text("プレビュー（共通）"),
    ...
    def build_infer_page():
        return ft.Column(
            [
                ...
                infer_parent,
                ft.Row([
                    ft.ElevatedButton(text="モデルロード", on_click=on_load_model),
                    ft.ElevatedButton(text="推論開始", on_click=on_start_infer),
                    ft.ElevatedButton(text="推論停止", on_click=on_stop_infer),
                ]),
                ft.Divider(),
                ft.Text("プレビュー（共通）"),
                preview,
