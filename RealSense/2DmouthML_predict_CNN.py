# -----------------------------------------
# 機能（soundML_predict_CNN.py と同じ構成）:
#  1) model_dir/model.pt, meta.json を読み込む
...
def collect_pngs(p: Path, use_views: Optional[List[str]]) -> list[Path]:
    if p.is_file():
        if p.suffix.lower() != ".png":
            return []
        return [p] if _view_filter_ok(p, use_views) else []
    if p.is_dir():
        items = sorted([x for x in p.glob("*.png") if x.is_file()])
        if use_views is not None:
            items = [x for x in items if _view_filter_ok(x, use_views)]
        return items
...
def main():
    ...
    inputs = collect_pngs(Path(args.input), use_views=use_views)
    if not inputs:
        raise ValueError("No png files found in input (or filtered out by use_views).")

    X = torch.stack([load_image_as_tensor(p, img_h, img_w, grayscale) for p in inputs], dim=0).to(device)

    with torch.no_grad():
        logits = model(X)                                  # (N,2)
        proba = torch.softmax(logits, dim=1).cpu().numpy() # softmax確率
    ...
    print("=== PREDICT DONE (MOUTH 2D CNN, U vs NotU) ===")
