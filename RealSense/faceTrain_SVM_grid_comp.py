...
# 被験者ごとのデータルートを複数指定（例）
DATA_ROOTS = [
    ("Subject1", Path(r"./YR/mouth_ply")),
    ("Subject2", Path(r"./NN/mouth_ply")),
    ("Subject3", Path(r"./NT/mouth_ply")),
    ("Subject4", Path(r"./KH/mouth_ply")),
    ("Subject5", Path(r"./SR/mouth_ply")),
]
...
OUT_PNG = OUT_DIR / f"grid_sweep_accuracy_subject_compare_{FEATURE_MODE}.png"
OUT_JSON = OUT_DIR / f"grid_sweep_accuracy_subject_compare_{FEATURE_MODE}.json"

# グラフ表示設定
SHOW_EACH_SUBJECT_LINES = True   # True: 被験者別ラインも表示 / False: 平均のみ
SHOW_MEAN_LINE = False            # 平均ライン
SHOW_MEAN_BAND = False           # 平均±標準偏差の帯
...
def plot_results(results_by_subject: dict, grids: list[int], out_png: Path):
    ...
    for g in grids:
        ...
        if len(vals) == 0:
            ...
            std_ys.append(float(np.std(vals, ddof=0)))

    # 平均が最大のGRID（ピックアップ）
    best_idx = int(np.nanargmax(mean_ys)) if np.any(np.isfinite(mean_ys)) else None
    best_grid = grids[best_idx] if best_idx is not None else None
    best_mean = mean_ys[best_idx] if best_idx is not None else None

    plt.figure(figsize=(12, 7))

    # 被験者別ライン
    if SHOW_EACH_SUBJECT_LINES:
        for subj, g2acc in results_by_subject.items():
            ys = [g2acc.get(g, np.nan) for g in grids]
            plt.plot(grids, ys, marker="o", linewidth=1, alpha=0.8, label=subj)

    # 平均±標準偏差の帯
    if SHOW_MEAN_BAND:
        lo = [m - s if np.isfinite(m) and np.isfinite(s) else np.nan for m, s in zip(mean_ys, std_ys)]
        hi = [m + s if np.isfinite(m) and np.isfinite(s) else np.nan for m, s in zip(mean_ys, std_ys)]
        plt.fill_between(grids, lo, hi, alpha=0.2)

    # # 平均ライン
    # if SHOW_MEAN_LINE:
    #     plt.plot(grids, mean_ys, marker="o", linewidth=3, label="Mean")

    # # best grid を縦線で明示
    # if best_grid is not None:
    #     plt.axvline(best_grid, linestyle="--")
    #     plt.text(best_grid, 0.0, f"  Best GRID={best_grid}\n  Mean={best_mean:.3f}",
    #              rotation=90, va="bottom")

    plt.xlabel("GRID size", fontsize=20)
    plt.ylabel("Test Accuracy", fontsize=20)
    plt.title(f"Grid Sweep (Test Accuracy) | mode={FEATURE_MODE}", fontsize=20)
    plt.xticks(fontsize=14)
    plt.yticks(fontsize=14)
    plt.grid(alpha=0.3)

    # 被験者数が多い場合に凡例が邪魔ならコメントアウト可
...
def main():
    ...
    for subj_name, data_root in DATA_ROOTS:
    ...
    out_obj = {
        ...
        "grids": GRIDS,
        "data_roots": [{"subject": name, "path": str(p)} for name, p in DATA_ROOTS],
        "runs": all_runs,
